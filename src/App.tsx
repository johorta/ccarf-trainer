...
function App() {
  ...
  const examLevelStats = examConfigs.map((exam) => {
    const attempts = progress.examAttempts.filter((attempt) => attempt.examId === exam.id)
    const points = attempts.map(scorePoints)
    return {
      id: exam.id,
      level: exam.difficulty,
      attempts: attempts.length,
      average: points.length ? Math.round(points.reduce((sum, value) => sum + value, 0) / points.length) : null,
    }
  })
  ...
  function scoreFor(topic: Topic) {
    const score = progress.byTopic[topic.id]
    return score ? (score.answered ? Math.round(score.correct / score.answered * 100) : 0) : topic.reportScore
  }
  ...
  function resetTopicScore(topic: Topic) {
    const topicScore = progress.byTopic[topic.id]
    if (!topicScore?.answered) return
    const confirmed = window.confirm(`¿Reiniciar el dominio estimado de "${topic.name}"?\n\nEsto borra solo el porcentaje de este tema. No elimina el historial de simuladores ni cambia la precisión global.`)
    if (!confirmed) return
    persist({
      ...progress,
      byTopic: {
        ...progress.byTopic,
        [topic.id]: { answered: 0, correct: 0 },
      },
    })
  }
  ...
  function finishExam() {
    ...
    const result: ExamAttempt = {
      ...
      title: `${activeExam.title} · ${activeExam.questionCount} preguntas`,
